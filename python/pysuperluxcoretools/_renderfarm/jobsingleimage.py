        ...
        if os.path.exists(self.workDirectory):
            ...
            if self.__CheckWorkDirectoryMD5AndSeed():
                ...
                for filmName in files:
                    ...
                    if filmThread := pysuperluxcore.Film(fullFileName):
...
class RenderFarmJobSingleImageThread:
    ...
    def NodeThread(self):
        ...
        try:
            ...
            socketutils.SendLine(nodeSocket, pysuperluxcore.Version())
            socketutils.RecvOk(nodeSocket)
            logger.info("Remote node has the same pysuperluxcore version")
