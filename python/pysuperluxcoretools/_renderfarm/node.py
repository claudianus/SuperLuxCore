...
logger = logging.getLogger(loghandler.loggerName + "._renderfarmnode")
...
class RenderFarmNode:
    def __init__(
        ...
        self.renderFarmNodeThread = None
    ...
    def __HandleConnection(self, clientSocket, addr):
        _id = str(uuid.uuid4())
        renderConfigFile = "renderfarmnode-" + _id + ".bcf"
        filmFile = "renderfarmnode-" + _id + ".flm"
        try:
            logger.info("Received connection from: %s", str(addr))

            # -----------------------------------------------------------
            # Check pysuperluxcore version
            # -----------------------------------------------------------

            remoteVersion = socketutils.RecvLine(clientSocket)
            logger.info("Remote pysuperluxcore version: %s", remoteVersion)
            logger.info("Local pysuperluxcore version: %s", pysuperluxcore.Version())
            if remoteVersion != pysuperluxcore.Version():
                logger.info("No matching pysuperluxcore versions !")
                socketutils.SendLine(
                    clientSocket,
                    "ERROR: wrong pysuperluxcore version" + pysuperluxcore.Version(),
                )
                return
            ...
            logger.info(
                "Receiving RenderConfig serialized file: %s", renderConfigFile
            )
            socketutils.RecvFile(clientSocket, renderConfigFile)
            logger.info(
                "Receiving RenderConfig serialized MD5: %s",
            ...
            logger.info("Received seed: %s", seed)
            seed = int(seed)
            ...
            logger.info(
                "Reading RenderConfig serialized file: %s", renderConfigFile
            )
            config = pysuperluxcore.RenderConfig(renderConfigFile)
            # Sanitize the RenderConfig
            __SanitizeRenderConfig(config)
            ...
            try:
                ...
                while not self.threadStop:
                    ...
                    if result is None:
                        continue
                    logger.info("Received command: %s", result)
                    ...
                    trianglecount = stats.Get(
                        "stats.dataset.trianglecount"
                    ).GetFloat()
                    fmt = (
                        "[Elapsed time: %3dsec][Samples %4d]"
...
def __SanitizeRenderConfig(config):
    # First sanitize the render engine type

    if (
        pysuperluxcore.GetPlatformDesc()
        .Get("compile.LUXRAYS_DISABLE_OPENCL")
        .GetBool()
    ):
        ...
        if props.IsDefined("renderengine.type"):
            ...
            if renderEngine == "PATHOCL":
                ...
                logger.info(
                    "rendernegine.type set to PATHOCL, switching to PATHCPU"
                )
            elif renderEngine == "TILEPATHOCL":
                # Switch to TILEPATHCPU
                renderEngine = "TILEPATHCPU"
                logger.info(
                    "rendernegine.type set to TILEPATHOCL, "
                    "switching to TILEPATHCPU"
                )
            elif renderEngine == "RTPATHOCL":
                # Switch to RTPATHCPU
                renderEngine = "RTPATHCPU"
                logger.info(
                    "rendernegine.type set to RTPATHOCL, "
                    "switching to RTPATHCPU"
                )

            config.Parse(
                pysuperluxcore.Properties().SetFromString(
    ...
    config.Parse(pysuperluxcore.Properties().SetFromString(defaultProps))
