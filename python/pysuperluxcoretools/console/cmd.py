...
logger = logging.getLogger(loghandler.loggerName + ".luxcoreconsole")


def BatchRendering(config, startState=None, startFilm=None):
    session = pysuperluxcore.RenderSession(config, startState, startFilm)
    ...
    try:
        while not session.HasDone():
            ...
            fmt = (
                "[Elapsed time: %3d/%dsec][Samples %4d/%d][Convergence %f%%]"
                "[Avg. samples/sec % 3.2fM on %.1fK tris]"
            )
            samplesec = stats.Get(
                "stats.renderengine.total.samplesec"
            ).GetFloat()
            trianglecount = stats.Get("stats.dataset.trianglecount").GetFloat()
            logger.info(
                fmt,
                elapsedTime,
                haltTime,
...
def LuxCoreConsole(argv):
    parser = argparse.ArgumentParser(
        prog="pysuperluxcore-console",
        description="PySuperLuxCoreConsole",
    )
    parser.add_argument(
        "fileToRender", help=".cfg, .lxs, .bcf or .rsm file to render"
    )
    parser.add_argument(
        "-f", "--scene", metavar="FILE_NAME", nargs=1, help="scene file name"
    )
    parser.add_argument(
        "-w",
        "--film-width",
    ...
    cmdLineProp = pysuperluxcore.Properties()
    if args.scene:
        cmdLineProp.Set(pysuperluxcore.Property("scene.file", args.scene))
    if args.film_width:
        cmdLineProp.Set(pysuperluxcore.Property("film.width", args.film_width))
    if args.film_height:
        cmdLineProp.Set(pysuperluxcore.Property("film.height", args.film_height))
    if args.define:
        for name, value in args.define:
            cmdLineProp.Set(pysuperluxcore.Property(name, value))
    if args.current_dir:
    ...
    if configFileNameExt == ".lxs":
        ...
        # Parse the LXS file
        configProps = pysuperluxcore.Properties()
        sceneProps = pysuperluxcore.Properties()
        pysuperluxcore.ParseLXS(args.fileToRender, configProps, sceneProps)
        ...
        config = pysuperluxcore.RenderConfig(configProps, scene)
    elif configFileNameExt == ".cfg":
        # It is a LuxCore SDL file
        configProps = pysuperluxcore.Properties(args.fileToRender)
        configProps.Set(cmdLineProp)
        config = pysuperluxcore.RenderConfig(configProps)
    elif configFileNameExt == ".bcf":
        # It is a LuxCore RenderConfig binary archive
        config = pysuperluxcore.RenderConfig(args.fileToRender)
        config.Parse(cmdLineProp)
    elif configFileNameExt == ".rsm":
        # It is a rendering resume file
        (config, startState, startFilm) = (
            pysuperluxcore.RenderConfig.LoadResumeFile(args.fileToRender)
        )
        config.Parse(cmdLineProp)
    else:
    ...
    if args.camera_shutter:
        cameraProps = (
            config.GetScene().ToProperties().GetAllProperties("scene.camera")
        )
        cameraProps.Set(
            pysuperluxcore.Property(
                "scene.camera.shutteropen", [args.camera_shutter[0]]
            )
        )
        cameraProps.Set(
            pysuperluxcore.Property(
                "scene.camera.shutterclose", [args.camera_shutter[1]]
            )
        )
    ...
    if not isFileSaver:
        config.Parse(
            pysuperluxcore.Properties().Set(
                pysuperluxcore.Property("screen.refresh.interval", 2500)
            )
        )
...
def main():
    ...
    try:
        ...
        pysuperluxcore.AddFileNameResolverPath(".")
