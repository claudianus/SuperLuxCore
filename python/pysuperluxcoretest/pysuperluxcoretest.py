...
def PropertiesTests():
    ...
    prop = pysuperluxcore.Property("test1.prop1", 0)
    prop.Set(0, 3705339624)

    prop = pysuperluxcore.Property("test1.prop1", 3705339624)
    ...
    print(
        "[%s]\n" % pysuperluxcore.Property("test1.prop1").Add([1, 2, 3]).Add([1.0, 2.0, 3.0]),
        flush=True,
    )
    ...
    for i in range(0, len(blob)):
        blob[i] = i
    prop = pysuperluxcore.Property("test.blob", [blob])
    prop.Add([[1, 2, 3]])
    ...
    prop = pysuperluxcore.Property("test.array", [])
    prop.AddAllFloat(a)
...
def LuxRaysDeviceTests():
    ...
    try:
        deviceList = pysuperluxcore.GetOpenCLDeviceList()
...
def SimpleRender():
    ...
    props = pysuperluxcore.Properties(os.path.join(PATH_TO_SCENE, "luxball-hdr.cfg"))
...
def GetOutputTest():
    ...
    while True:
        ...
        # This is mostly for testing the PySuperLuxCore functionality, save an image every second

        # Update the image
        session.GetFilm().GetOutputFloat(
            pysuperluxcore.FilmOutputType.RGB_IMAGEPIPELINE, imageBufferFloat
        )
        pysuperluxcore.ConvertFilmChannelOutput_3xFloat_To_4xUChar(
...
def ExtractConfiguration():
    ...
    # Load the configuration from file
    props = pysuperluxcore.Properties(os.path.join(PATH_TO_SCENE, "luxball-hdr-comp.cfg"))
...
def StrandsRender():
    ...
    cfgProps.Set(pysuperluxcore.Property("renderengine.type", ["PATHCPU"]))
    ...
    # Create the scene
    # resizePolicyProps = pysuperluxcore.Properties()
    ...
    #    """)
    # scene = pysuperluxcore.Scene(scnProps, resizePolicyProps)
    ...
    strandsProps = pysuperluxcore.Properties()
...
def SaveResumeRenderingM():
    ...
    session = pysuperluxcore.RenderSession(config, "test.rst", "test.flm")
...
def SaveResumeRenderingS():
    ...
    (config, startState, startFilm) = pysuperluxcore.RenderConfig.LoadResumeFile("test.rsm")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("LuxCore %s" % pysuperluxcore.Version(), flush=True)
        ...
        final_folder = os.path.join(current_path, "pysuperluxcoretest-out")
        ignore_files = shutil.ignore_patterns("pysuperluxcoretest.py", "scenes", "__*")
