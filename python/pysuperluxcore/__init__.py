...
_OIDN_PATHS = {
    "Linux": (_LUXFOLDER / ".." / "pysuperluxcore.oidn", "oidnDenoise"),
    "Windows": (_LUXFOLDER / ".." / "pysuperluxcore.libs", "oidnDenoise.exe"),
    "Darwin": (_LUXFOLDER / ".." / "pysuperluxcore.oidn", "oidnDenoise"),
}

# Keep nvrtc handles alive during all pysuperluxcore life...
_NVRTC_HANDLES = []
...
def ensure_nvrtc():
    """Ensure nvidia runtime compiler will be available for pysuperluxcore.

    nvrtc is provided by 'nvidia-cuda-nvrtc' package from Pypi, via pysuperluxcore
    ...
    # as a pysuperluxcore dependency (see pyproject.toml)
...
def try_load_libs(name, prefix):
    """Ensure libs are loadable, in the right version for pysuperluxcore."""
    if platform.system() != "Linux":
        return  # Only for Linux

    path_to_libs = _LUXFOLDER / ".." / "pysuperluxcore.libs"
