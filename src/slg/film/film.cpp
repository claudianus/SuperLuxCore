...
FilmUPtr Film::Create(
	const u_int width,
	const u_int height,
	const u_int *subRegion
) {
	auto result = std::make_unique<Film>(Private(), width, height, subRegion);
	result->InitFilmDenoiser();
	return result;
}

void Film::InitFilmDenoiser() {
	filmDenoiser = std::make_unique<FilmDenoiser>(FilmPtr(this));
}
...
void Film::CopyHaltSettings(const Film &film) {
	...
	if (film.convTest) {
		...
		convTest = new FilmConvTest(
			FilmPtr(this), haltNoiseThreshold, haltNoiseThresholdWarmUp,
	...
	if (film.noiseEstimation) {
		...
		noiseEstimation = new FilmNoiseEstimation(
			*this,
			noiseEstimationWarmUp,
			noiseEstimationTestStep,
			noiseEstimationFilterScale,
