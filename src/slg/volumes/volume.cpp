...
SchlickScatter::SchlickScatter(VolumeConstRef vol, TextureConstRef gTex,
		const bool hg) :
	volume(vol), g(gTex), useHG(hg) {
}

// Henyey-Greenstein phase function value, spectral g. The denominator uses
// +2*g*cos instead of -2*g*cos because localEyeDir is reversed compared to
// the standard phase function definition.
static Spectrum HGValue(const Spectrum &g, const float cosTheta) {
	const Spectrum gg = g * g;
	const Spectrum denom = Spectrum(1.f) + gg + 2.f * g * cosTheta;
	return (Spectrum(1.f) - gg) / (denom * denom.Sqrt() * (4.f * M_PI));
}
...
Spectrum SchlickScatter::GetColor(const HitPoint &hitPoint) const {
	...
	for (u_int i = 0; i < COLOR_SAMPLES; ++i) {
		if (r.c[i] > 0.f)
			...
			// Purely absorbing point (sigma_s = 0, sigma_a > 0): such vertices
			// can be generated by sigma_t-rate delta tracking and must have
			// zero albedo. The sigma_a = 0 corner keeps 1 for preview.
			r.c[i] = (sigmaA.c[i] > 0.f) ? 0.f : 1.f;
	}
...
Spectrum SchlickScatter::Evaluate(
	...
	const Spectrum gValue = GetG().GetSpectrumValue(hitPoint).Clamp(
			useHG ? -0.999f : -1.f, useHG ? 0.999f : 1.f);
	...
	*event = DIFFUSE | REFLECT;
	...
	if (useHG) {
		const Spectrum value = HGValue(gValue, dotEyeLight);
		const float pdf = HGValue(Spectrum(gValue.Filter()), dotEyeLight).Filter();
		if (pdf <= 0.f)
			return Spectrum();
		if (directPdfW)
			*directPdfW = pdf;
		if (reversePdfW)
			*reversePdfW = pdf;
		return GetColor(hitPoint) * value;
	}
...
Spectrum SchlickScatter::Sample(const HitPoint &hitPoint,
	...
	const Spectrum gValue = GetG().GetSpectrumValue(hitPoint).Clamp(
			useHG ? -0.999f : -1.f, useHG ? 0.999f : 1.f);
	const float gFilter = gValue.Filter();
	...
	float cost;
	if (useHG) {
		// Henyey-Greenstein inversion. The leading - converts from the
		// standard scattering-angle convention to the reversed localEyeDir
		// convention used here (see Evaluate()).
		if (fabsf(gFilter) > 1e-5f) {
			const float s = (1.f - gFilter * gFilter) / (1.f - gFilter + 2.f * gFilter * u0);
			cost = -(1.f + gFilter * gFilter - s * s) / (2.f * gFilter);
		} else
			cost = -(1.f - 2.f * u0);
	} else {
		// Add a - because localEyeDir is reversed compared to the standard
	...
	*localSampledDir = SphericalDirection(sqrtf(Max(0.f, 1.f - cost * cost)), cost,
	...
	if (useHG)
		*pdfW = HGValue(Spectrum(gFilter), cost).Filter();
	else {
...
void SchlickScatter::Pdf(
	...
	float pdf;
	if (useHG)
		pdf = HGValue(Spectrum(gValue.Filter()), dotEyeLight).Filter();
	else {
		const float kFilter = Spectrum(gValue * (Spectrum(1.55f) - .55f * gValue * gValue)).Filter();
		// 1+k*cos instead of 1-k*cos because localEyeDir is reversed compared
		// to the standard phase function definition
		const float compcostFilter = 1.f + kFilter * dotEyeLight;
		pdf = (1.f - kFilter * kFilter) / (compcostFilter * compcostFilter * (4.f * M_PI));
	}
