...
#
# Screen-space neighbour-pixel merge (E2b): each shade point merges up
# to 2 pseudo-random neighbour reservoirs from the per-pixel reservoir
# buffer (5x5 window, same-surface geometric gate on hit point + landing
# normal) with GRIS weights b = wSum * (pi_new / pi_old). This replaces
# the earlier world-space hash grid, which merged unrelated surfaces and
# measured ~1.1-1.3x worse RMSE.
#
# T1 (unbiasedness): RESTIR+spatial mean vs LOG_POWER reference mean.
# T2 (variance): pixel-space merges reuse same-surface winners, so the
#    target ratio stays near 1; assert RMSE does not regress vs no
#    spatial reuse.
...
# watchdog panic (observed during development). Small task count + a
# hard per-render deadline instead of an infinite wait.
TASK_COUNT = 8192
