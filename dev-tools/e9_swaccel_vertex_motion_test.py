#!/usr/bin/env python3
"""E9 regression test: software MBVH kernel (OpenCL / cl2msl) per-vertex
deformation motion blur.

Same deterministic scene as e9_metal_vertex_motion_test.py, but the
native Metal HWRT path is disabled via LUXRAYS_METAL_HWRT=0, forcing the
software Accelerator_Intersect_RayBuffer kernel (mbvh.cl — translated to
MSL by cl2msl on this platform, identical code to the OpenCL path).

Validates the Phase-3 swept-bound software path: swept leaf BVH bounds,
vertex interpolation at ray->time, correct hit attribution, clamping,
...
def main():
    ...
    check("static quad visible (sw kernel)", c is not None and 1000 < c[2], str(c))
    if c is None:
