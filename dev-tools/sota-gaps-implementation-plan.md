...
1. **Wavefront M3** — compaction+정렬+기본 활성화 (GPU 대형씬 스루풋)
   → **M3a 랜딩** (`37cc04b4b`, 디바이스 QueuePrefix — cornell ~2.5x)
2. **Cryptomatte** — 모든 프로덕션 보유, 미구현
   → **✅ 랜딩** (`aabfdb3e5`, `CRYPTOMATTE_OBJECT`/`_MATERIAL`)
3. **Light linking** — 라이트↔오브젝트 조명 관계 필터
   → **✅ 랜딩** (`8d05a4ef1` 64비트 그룹 + `8d05a4ef1`, BLC UI 완비)
4. **LPE** (light path expression) — 확장 AOV 분해
   → **✅ 랜딩** (`7a1622a86`, bounded NFA `film.lpe.N.*`)
5. **Path guiding 성숙도** — OpenPGL 검증 아이디어 선별 이식
   → **✅ P5 랜딩** (`00804937b`, 정식 `path.guiding.*`+BIC-K+폴백+영속성)
...
| 갭 | 현재 상태 | 결정적 사실 |
|---|---|---|
| Wavefront | **M3a 랜딩** — 디바이스 QueuePrefix로 host sync 2→1 (`37cc04b4b`) | cornell 720²/30s Metal: dense ~9.5M vs wavefront ~24M samples/s (**~2.5x 역전** — 구 −7~−17% 판정은 M3a 이전 측정). totals readback(72B/iter)은 런치 사이징에 필수. dense 디폴트 유지 |
...
### 1.2 M3a: 호스트 동기화 제거 — ✅ 랜딩 (`37cc04b4b`, 2026-09-25)
...
| P1 | Cryptomatte (object+material) | P0 | ✅ 랜딩 `aabfdb3e5` — e40 PASS |
| P2 | Light linking (illumination, flat+BVH) | — | ✅ 랜딩 `8d05a4ef1`+`8d05a4ef1` — e41 8/8 |
| P3 | Wavefront M3a (sync 제거) | — | ✅ 랜딩 `37cc04b4b` — cornell ~2.5x. 단, totals readback은 유지(stale sizing ~6x 회귀 실측 — §1.2의 "정확성 무손실"은 맞으나 진행 지연 비용이 컸음) |
| P4 | LPE (eye-path 완전) | P0(채널) | ✅ 랜딩 `7a1622a86` — e42 PASS. 범위: PATHCPU/PATHOCL |
| P5 | Guiding 5.1+5.2+5.5 (fallback, adaptive K, 프로퍼티화) | — | ✅ 랜딩 `00804937b` — 계층 폴백+BIC-K+`path.guiding.*` 정식화, e43 PASS |
