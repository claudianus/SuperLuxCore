...
| mathfunc texture | [textures.md](textures.md) | `13afd4f29` | `dev-tools/e10_mathfunc_test.py` (14 checks) | CPU/OCL/Metal |
| Gabor noise texture | [textures.md](textures.md) | `c34efab71` | `dev-tools/e11_gabor_test.py` (14 checks) | CPU/OCL/Metal |
...
| Metal native curves | [../dev-tools/metal_curve_design.md](../dev-tools/metal_curve_design.md) | `4a90e7057`; float3 packing `fd3f60471`; gpuAddress residency fix `fd3f60471` | scenes/strands/hair.scn + `dev-tools/e22_metal_curve_test.py` (coverage/parity/indirect gates) | **Apple only** (Metal HWRT) |
...
| Blender adapter | [blender-adapter.md](blender-adapter.md) | SuperBlendLuxCore repo — motion blur `10630c41a`, `10630c41a`; persistent-scene export (A6-II) `10630c41a` + frame-change fix `10630c41a` + regression `10630c41a`, `10630c41a`; material+geometry deltas (A6-III) `10630c41a`, `10630c41a` | .blend scenes; `SuperBlendLuxCore/dev-tools/a6_persistent_scene_test.py` | all; Metal opt = Apple |
| Wavefront task queues (M1/M2/M3a, opt-in) | [../dev-tools/wavefront-design.md](../dev-tools/wavefront-design.md) | `ae418f869` cl2msl fix, `ae418f869` M1, `ae418f869` M2 λ-bucketed queues, `37cc04b4b` M3a device prefix (branch `feature/wavefront-queues`) | cornell, cornell-spectral | OCL/Metal; `LUXRAYS_WAVEFRONT_QUEUES=1` |
| Deformation motion blur (E9, scoped) | [../dev-tools/deformation-motion-blur-design.md](../dev-tools/deformation-motion-blur-design.md) | `cf0db7262` design doc; `25472b8a5` plumbing; `25472b8a5` Metal HWRT; `25472b8a5` SW MBVH; `25472b8a5` Embree; SuperBlendLuxCore `c328fa1c5` export | `vertexmotion_test`; `dev-tools/e9_*_vertex_motion_test.py`; SuperBlendLuxCore `dev-tools/e9_vertex_motion_e2e_test.py` | Metal HWRT + SW MBVH (OCL/cl2msl) + Embree timesteps; mesh export (hair pending) |
| Adaptive caustic partition | [adaptive-caustic.md](adaptive-caustic.md) | `038414538` | `scenes/cornell/caustic-roughglass.scn`; `dev-tools/e25_adaptive_caustic_test.py`, `e25_render_compare.py`, `e25_caustic_channel.py` | CPU/OCL/Metal |
| GPU light tracing + LMNEE/MGE | [gpu_lighttracing.md](gpu_lighttracing.md) | `45caf3459` tasks, `45caf3459` Emit, `45caf3459` camera, `45caf3459` state machine, `038414538` splat, `45caf3459` LMNEE, `038414538` MGE, `345431a45` lt-only natives | `dev-tools/e23_*` LT parity; `e45_lighttracing_only_test.py`; cornell | OCL/Metal |
| Path guiding M4a–P5 | [path-guiding.md](path-guiding.md) + [path-guiding-m4-design.md](path-guiding-m4-design.md) | `065f895d7` SD-tree, `065f895d7` RIS, `065f895d7` portal, `c92c2f340` GPU field, `c92c2f340` GPU RIS, `c92c2f340` GPU portal, `00804937b` P5 (`path.guiding.*` + BIC-K + fallback) | `dev-tools/e26_*`, `e27_*`, `e43_*` | CPU/OCL/Metal |
| Volume tracking (E6) | [volume-tracking.md](volume-tracking.md) | `ba793a22f` null-collision, `ba793a22f` residual, `ba793a22f` minorant, `ba793a22f`/`ba793a22f` equiangular MIS, `ba793a22f` HG, `ba793a22f` contribution-aware, `632e8928e` flight | `dev-tools/e34_volume_guiding_parity.py`, `pyunittests/pysuperluxcoreunittests/tests/materials/testvolumetracking.py` | CPU/OCL/Metal |
| Light BVH strategy | `../../dev-tools/lightstrategy-audit.md` + [../engineering/light-bvh.md](../engineering/light-bvh.md) | `6c0006bc4` (`lightstrategy.type=LIGHT_BVH`, E&K'18) | `dev-tools/e26_lightbvh_test.py`, e37 audit | CPU/OCL/Metal |
| GPU vertex connection (M6) | [gpu_vertexconnection.md](gpu_vertexconnection.md) | `bc6364314`, MIS lift gate `345431a45` | `dev-tools/e38_vc_smoke.py`, `e44_vc_shadowtransparency_test.py` | OCL/Metal |
| GPU vertex merging (M7) | [gpu_vertexconnection.md](gpu_vertexconnection.md) | `b780648e4`, scale fix `d4a5282f3`, radius `d4a5282f3` | `dev-tools/e38_*`, `e39_bidir_focus.py` | OCL/Metal |
| Vulkan backend + HWRT | [vulkan-backend-assessment.md](vulkan-backend-assessment.md) + `../../dev-tools/vkrt/` | M0/M1 spike; M2 `ef7a4ca60` BLAS/TLAS+ray_query, `13208166f` maxVertex, `ef7a4ca60` AS rebuild, `bfc37876b` pipeline cache | `dev-tools/vk_intersect_test.cpp` (C++ target), `vk_rt_update_test.py`, `vulkan-regression.sh --full` (720p PATHOCL render) | MoltenVK (experimental — native drivers unvalidated) |
| Cryptomatte AOV | [../engineering/cryptomatte.md](../engineering/cryptomatte.md) | `aabfdb3e5` (`CRYPTOMATTE_OBJECT`/`_MATERIAL`, MurmurHash3 manifest) | `dev-tools/e40_cryptomatte_test.py` + visual | CPU/OCL/Metal |
| LPE AOV | [../engineering/lpe.md](../engineering/lpe.md) | `7a1622a86` (`film.lpe.N.expression`, bounded NFA ≤8) | `dev-tools/e42_lpe_test.py` + `e42_lpe_visual.py` | PATHCPU/PATHOCL only (no BIDIR/LT) |
...
| OpenPBR (S1) | [disney.md](disney.md) / material defs | OpenPBR material + parser + BLC node; Metal fix `fe11c2ee9` | `dev-tools/e31_openpbr_parity.py` | CPU/OCL/Metal |
| Random-walk SSS (S2/E35) | material defs / [../engineering/openpbr-sss-findings.md](../engineering/openpbr-sss-findings.md) | `62366b35b` albedo/mfp | `dev-tools/e35_sss_albedo.py`, `e35_visual_demo.py` | CPU/OCL/Metal |
...
> Maintenance: `cf0db7262` ExtMeshProp sizeless-layer fix (E7 fallback
> regression), `cf0db7262` robin_hood→tsl::robin_map, `cf0db7262` dead
> vendored assets (~47MB). Regression automation:
> `dev-tools/wavefront-regression.sh` (`ae418f869`, strands_hair case
> `cf0db7262`) and `dev-tools/parity-regression.sh` (`03dc67b74`, CPU/GPU
...
- ~~**Known portability fragility**~~ — fixed in `cf0db7262`:
...
- **Metal curves**: native Catmull-Rom primitives now replace the hair
  tessellation on the Metal HWRT path (commit `4a90e7057`, gated on
...
- **PATHCPU-vs-PATHOCL brightness difference** on bright emissives — root
  cause found and fixed in commit `4a90e7057`. It was not an emission
...
- **Wavefront queues (B2/E3 M1/M2/M3a)**: opt-in per-state task queues for the
  ...
  queue integrity clean, SOBOL/RANDOM/METROPOLIS). M3a (`37cc04b4b`)
