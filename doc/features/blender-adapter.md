...
| Cycles/Eevee compat | `a9d109d38` compat layer | Existing Cycles node trees re-interpreted without rebuild |
| Materials | Principled v2 + Disney transmission + thin-film, **OpenPBR material node** (S1), **Hair node with Chiang/Huang select** (`2ce4c2119`) | First-class mappings, no mix hacks |
| Objects | VOLUME(OpenVDB)/POINTCLOUD/displacement/dupli/instances, **`.lxm` mesh proxy + auto-proxy** (`superluxcore.bake_lxm_proxy`, `config.proxy_auto*`, cluster stride UI `73c655a7b`) | Evaluated-geometry bake, demand-paged .lxm load |
...
| Render features UI | GPU light tracing + LT-only, caustic focus/MGE, adaptive caustics, ReSTIR GI/visibility toggles, **vertex connection controls** (`2ce4c2119`/`2ce4c2119`), **P5 path-guiding controls** (`901015743`), **adaptive robust clamping** (`901015743`), **Vulkan backend + device select** (`73c655a7b`) | Artist-first panels; engine features exposed promptly |
| View layer / AOV | Cryptomatte toggles (`5a968f91d`), **LPE outputs** (`901015743`), light-linking UI + instanced coverage (`a9d109d38`/`901015743`/`901015743`), view-layer flags + opt-in halt overrides (`a9d109d38`), temporal accumulation panel | LPE: `film.lpe.N.*`; Cryptomatte: object/material |
| UX | Quick Setup 2.0 + scene-aware auto config (`5a968f91d`/`901015743`), auto light strategy/clamp/device, low-VRAM profile, quality presets, convergence stat row | Post-restore property trap documented in `SuperBlendLuxCore/doc/engineering/render-session.md` |
...
| Branding | `50dccfe21` extension/engine rebrand to SuperLuxCore, `50dccfe21` smoke test | `extensions/user_default/superluxcore`, `pysuperluxcore` wheel |
