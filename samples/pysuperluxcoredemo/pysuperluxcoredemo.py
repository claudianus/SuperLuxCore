...
def GetOutputTest():
	...
	while True:
		...
		# Update the image
		session.GetFilm().GetOutputFloat(pysuperluxcore.FilmOutputType.RGB_IMAGEPIPELINE, imageBufferFloat)
		pysuperluxcore.ConvertFilmChannelOutput_3xFloat_To_4xUChar(filmWidth, filmHeight,
...
def ExtractConfiguration():
	...
	# Load the configuration from file
	props = pysuperluxcore.Properties("scenes/luxball/luxball-hdr-comp.cfg")
...
def SaveResumeRenderingM():
	...
	session.GetFilm().SaveOutput("test-save.png", pysuperluxcore.FilmOutputType.RGB_IMAGEPIPELINE, pysuperluxcore.Properties())
	...
	session.GetFilm().SaveOutput("test-resume.png", pysuperluxcore.FilmOutputType.RGB_IMAGEPIPELINE, pysuperluxcore.Properties())
	session.Stop()
